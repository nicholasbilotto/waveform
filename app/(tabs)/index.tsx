...
const getTimelinePosition = (forecast: any) => {
    if (!forecast?.ai_analysis?.optimal_time) return { left: "25%", width: "30%" };
...
// One swipe page. Renders the full forecast card, a spinner, or a placeholder.
function SpotCard({ forecast, loading, slug, onRefresh, refreshing }: SpotCardProps) {
    // Loading state (lazy single-spot fetch in flight)
    if (loading) {
        return (
    ...
    // No data yet and not loading (e.g. just became visible, fetch about to start)
    if (!forecast) {
        const spotName = slug
    ...
    const timelinePos = getTimelinePosition(forecast);
...
export default function HomeScreen() {
    ...
    // --- PAGER DATA ---
    // Page 0 is the winner (already-loaded daily forecast), then the rest of the favorites.
    const winnerSlug: string | undefined = forecast?.spot_info?.slug;
    const pageData: string[] = winnerSlug
        ? [winnerSlug, ...favoriteSpots.filter((s) => s !== winnerSlug)]
        : favoriteSpots;

    // Lazy-load the visible non-winner page + pre-fetch its right neighbor.
    const onViewableItemsChanged = useRef(({ viewableItems }: any) => {
        ...
        const ensure = (i: number) => {
            if (i <= 0 || i >= pageDataRef.current.length) return; // index 0 is the winner
            const s = pageDataRef.current[i];
            if (s && !spotDetailsRef.current[s]) fetchSpotDetailRef.current(s);
        };
        ensure(idx);
        ensure(idx + 1); // look ahead one
    }).current;

    // Refs so the stable onViewableItemsChanged closure sees fresh values.
    const pageDataRef = useRef(pageData);
    pageDataRef.current = pageData;
    const spotDetailsRef = useRef(spotDetails);
    spotDetailsRef.current = spotDetails;
    const fetchSpotDetailRef = useRef(fetchSpotDetail);
    fetchSpotDetailRef.current = fetchSpotDetail;
    ...
    const renderItem = useCallback(
        ({ item: slug, index }: { item: string; index: number }) => (
            <SpotCard
                slug={slug}
                forecast={index === 0 ? forecast : spotDetails[slug]}
                loading={index === 0 ? false : !!spotDetailLoading[slug]}
                onRefresh={onRefresh}
                refreshing={refreshing}
            />
        ),
        [forecast, spotDetails, spotDetailLoading, refreshing]
    );

    // The header reflects the currently-viewed spot.
    const activeSlug = pageData[activeIndex];
    const activeForecast = activeIndex === 0 ? forecast : spotDetails[activeSlug];
    const isWinnerActive = activeIndex === 0;
    const headerDayText = activeForecast?.ai_analysis?.is_tomorrow ? "Tomorrow" : "Today";
    const fallbackName = activeSlug
        ? activeSlug.split("-").map((w) => w.charAt(0).toUpperCase() + w.slice(1)).join(" ")
        : "";
    const headerName = activeForecast?.spot_info?.name || fallbackName;
    ...
    const renderContent = () => {
        ...
        return (
            <View className="flex-1">
                                ...
                                <View>
                                    <View className="flex-row items-center mb-1">
                                        {isWinnerActive ? (
                                            ...
                                            <Text className="text-cyan-400 text-[10px] font-black uppercase tracking-[2px]">
                                                Favorite {headerDayText}
                                            </Text>
                                        )}
                                    </View>
                ...
                {/* Page indicator dots */}
                {pageData.length > 1 && (
                    <View className="flex-row items-center justify-center pb-3 pt-2">
                        {pageData.map((slug, i) => (
                            <View
                                key={slug}
